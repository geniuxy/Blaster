	...
	float HitTime)
{
	...
	if (PlayerCharacter && HitCharacter && PlayerCharacter->GetEquippedWeapon() && Confirm.bHitConfirmed)
	{
		const float Damage = Confirm.bHeadShot
			                     ? PlayerCharacter->GetEquippedWeapon()->GetHeadShotDamage()
	...
	float HitTime)
{
	...
	{
		if (HitCharacter == nullptr || PlayerCharacter == nullptr || PlayerCharacter->GetEquippedWeapon() == nullptr)
			continue;
		float TotalDamage = 0.f;
		if (Confirm.HeadShots.Contains(HitCharacter))
		{
			float HeadShotDamage =
				Confirm.HeadShots[HitCharacter] * PlayerCharacter->GetEquippedWeapon()->GetHeadShotDamage();
			TotalDamage += HeadShotDamage;
		}
		if (Confirm.BodyShots.Contains(HitCharacter))
		{
			float BodyShotDamage = Confirm.BodyShots[HitCharacter] * PlayerCharacter->GetEquippedWeapon()->GetDamage();
		...
		else // didn't hit head, check the rest of the boxes
		{
			for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
			{
				if (HitBoxPair.Value != nullptr)
				{
					...
					HitBoxPair.Value->SetCollisionResponseToChannel(ECollisionChannel::ECC_HitBox,
					                                                ECollisionResponse::ECR_Block);
				}
			}

			World->LineTraceSingleByChannel(ConfirmHitResult, TraceStart, TraceEnd, ECC_HitBox);
			if (ConfirmHitResult.bBlockingHit) // we hit the other part
			{
				// if (ConfirmHitResult.Component.IsValid())
				// {
				// 	UBoxComponent* Box = Cast<UBoxComponent>(ConfirmHitResult.Component);
...
{
	if (PlayerCharacter == nullptr || !PlayerCharacter->HasAuthority()) return;
